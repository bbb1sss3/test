/* Source File Upload Time : 10-21-20 12:03:18 PM*/


/* Source File Upload Time : 8-24-20 2:18:13 PM*/


/* Source File Upload Time : 5-7-20 12:49:37 PM*/


/* Source File Upload Time : 3-13-20 5:24:09 PM*/


/* Source File Upload Time : 10-20-19 2:29:27 PM*/


/* Source File Upload Time : 9-17-19 11:38:13 AM*/


/* Source File Upload Time : 2019-08-27 10:35:56 AM*/


/* Source File Upload Time : 2019-08-26 11:47:40 AM*/
...
//$("[name=ed_val]", _doc.element).attr({'placeholder' : $fn.getCodeMsg("aprv_sub_193.title.a27")   }); 
...
(function(_$$, $){
	_$$.aprv_sub193 = {
			subdoc : {
					SUBNAME				: "sub193"	
					, init : function($doc) {
							var _me = _$$.aprv_sub193.subdoc, opt = $doc.options;
							var el = $doc.element;
							var _$table01 = $("table[name=sub193_Table01]", $doc.element);
                            ...
                            $('#basicExample1').timepicker(); 
							  ...
							  $("#_pop").on("click", function(){
                                    _me.AprDocTransfer($doc,'request_transfer_sanae')
				        		
				        		});
								$("input[name=Urgency1_1]").click(function () {

								    var resultValue = $("input[name=Urgency1_1_Nm]").val();

								     if (resultValue == 'LDAP') {
										 var _acode;
										 if($("input[name=_AREACODE]").val()=="E1"){
											 _acode="e"
										 }else if($("input[name=_AREACODE]").val()=="C1"){
											 _acode="c"
										 }else if($("input[name=_AREACODE]").val()=="S1"){
											 _acode="s"
										 }else if($("input[name=_AREACODE]").val()=="R1"){
											 _acode="r"
										 }
									$("input[name=ed_val_1_1]").val(_acode+$("input[name='_SABUN']").val()); //useranme
									$("input[name=ed_val_15_1]").val("PC Log-in Password");//password
								   } else {
									$("input[name=ed_val_1_1]").val("");
									$("input[name=ed_val_15_1]").val("");
								...
								if (opt.isnew) {
								 ...
								 $("input[name='Reason_1_2_2']").val($dwp.core.lang.getCurMsg(_org.oinfo.username));
								$("[name=_USER]", _$table01).val($dwp.core.lang.getCurMsg(_org.oinfo.username));
								$("[name=_SABUN]", _$table01).val($dwp.core.lang.getCurMsg(_org.oinfo.rempno));
								//$("[name=_APRTITLE]", _$table01).val($dwp.core.lang.getCurMsg(_org.oinfo.pos));
								$("[name=_DEPTNAME]", _$table01).val($dwp.core.lang.getCurMsg(_org.oinfo.orgname));
								//$("[name=_AREACODE]", _$table01).val($dwp.core.lang.getCurMsg(_org.oinfo.workareacode));
										//alert("@@")_AREACODE
								$("input[name=_APRTITLE]").val($("input[name='Titlename_1']").val());		
							       		...
							       		$fn.orgsel($("[name='org']", el), {
							       		    ...
							       		    autoseletcomplete: function (event, ui, doc) {
												 ...
												 $("input[name='_AREACODE']").val($dwp.core.lang.getCurMsg(ui.item.value.workareacode));
							       		    ...
							       		    orgselectcomplete: function (dialog, rtndata, doc) {
												...
												$("input[name='_AREACODE']").val($dwp.core.lang.getCurMsg(rtndata.list[0].workareacode));
							       		    },
							       		    fld: "User",
							       		    count: 1
							       		});
							
						$("#ed_193link").on("click", function () {
								window.open("http://gw.densokorea.com/dwp/aprv/com/aprvstart.nsf/form193?OpenPage", "_blank");
							});
					}
	,
            ...
            AprDocTransfer: function ($doc, act) {
                    ...
                    if (data.hasOwnProperty("result")) {
                        if (data.result >= "200" && data.result < "300") {
                           ...
                           //alert(data.subject)
							...
							//_linkhtml += " <br><font>의 비문(PC정보누설대책TOOL)설치 제외 신청 건이 처리 완료 되었습니다." + "</font>";
						//	_linkhtml += " <br><font>사용기간 : " + data.FromDate+" ~ "+data.ToDate+"" + "</font>";
                            //_linkhtml += "</a>";
							//_linkhtml += " <br><font>사용처(수령자) : " + data.Reason+" 관련업무 "+data.Reason_1+"" + "</font>";
							//_linkhtml += " <br><font>용도사유 : " + data.Reason_1_1+ "</font>";
							//_linkhtml += " <br><font>파일수 : " + data.Reason_2+"EA "+" 파일총용량 "+data.Reason_1_2+"MB" + "</font>";
							//_linkhtml += " <font>" + "의 Internet사용 신청 건이 처리 되었습니다." + "</font>";
                            _linkhtml += "</p>";                          
                            _linkhtml += " <br><font>제목: " + data.subject + "</font>";
                            _linkhtml += "<p style='font-family:맑은 고딕;font-size:10pt;LINE-HEIGHT: 120%; TEXT-INDENT: 0pt; MARGIN: 0px 0pt'>";
                            //_linkhtml += "(위 링크는 " + data.startdate + "부터 15일간 유효하며, 그 이후에는 연결이 되지 않습니다.)";
                            _linkhtml += "</p>";
                               _linkhtml += " <br><font>" + "신청하신 사내정보망신청건이 처리 되었습니다." + "</font>";
                            _linkhtml += "<p style='font-family:맑은 고딕;font-size:10pt;LINE-HEIGHT: 120%; TEXT-INDENT: 0pt; MARGIN: 0px 0pt'>&nbsp;</p>";
                            //수신인 없는 메일 작성화면 팝업
                            $fn.mailSend(user3[0], _linkhtml);
                        }
                    }
                }
				...
				, save : function($doc,opt){
                    ...
                    if ($("input[name='basicExample']",el).xval() == "") {
         				$fn.alert({msg : $fn.getCodeMsg("aprv_sub_193.msg.a1")});   //촬영일시 
         				return false;
         			}
					if ($("input[name='basicExample1']",el).xval() == "") {
         				$fn.alert({msg : $fn.getCodeMsg("aprv_sub_193.msg.a1")});   //촬영일시 
         				return false;
         			}
                    
                 
                     if ($("input[name='ed_val_1']",el).xval() == "") {
         				$fn.alert({msg : $fn.getCodeMsg("aprv_sub_193.msg.a2")});    
         				return false;
         			}
					       if ($("input[name='ed_val']",el).xval() == "") {
         				$fn.alert({msg : $fn.getCodeMsg("aprv_sub_193.msg.a3")});    
         				return false;
         			}
					if ($("input[name='Urgency1_Nm']",el).xval() == "") {
         				$fn.alert({msg : $fn.getCodeMsg("aprv_sub_193.msg.a4")});   //촬영일시 
         				return false;
         			}
