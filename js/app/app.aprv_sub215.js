	...
	_$$.aprv_sub215 = {
		...
		load: function ($doc) {
			...
			if (_opt.isnew) {
				$fn.xAjax({
					...
					console.log("처리", data);
					//$("[name=MISDocID]", el).val(paramDocID);
					$("[name=MISLogDocID]", el).val(data.misdocid);
					$("[name=MISLogDBPath]", el).val(data.misLogDb);
					$("[name=Body1]", el).val(data.Body1);
					$("[name=Body2]", el).val(data.Body2);
					var joString1 = data.Body1;
					joString1 = joString1.replace(/\n/gi, "");

					/* 1.기본정보 */
					console.log(joString1)
					$("[name=erpBodyDispDiv]", el).html(joString1)
					//$("#erpBodyDispDiv").html(joString1);
					joString1 = $("[name=erpBodyDispDiv]", el).text();
			...
			$('[name="role_btn_add"]', el).on('click', function (e) {
				...
				console.log("품의서추가클릭")
				_$$.aprv_sub215.selectRequestForm1($doc, "품의서", "AF155");
			});
			...
			function fnListType1(jsonResultSet, iDefaultRow) {
				...
				var sum1 = parseInt(jsonResultSet[0].FIELD01);
				var sum2 = parseInt(jsonResultSet[0].FIELD02);
				var seq = 1;
				console.log(jsonResultSet.length);
				for (i = 1; i < jsonResultSet.length; i++) {
					...
					if (seq == parseInt(jsonResult.FISEQ)) {
						//htmlString += "<tr><td class='clContentTdCenter' colspan='8' style='height:0px;padding:0px'></td></tr>";
						seq++;
					}

					var vFIELD20 = jsonResult.FIELD20 == "" ? "" : format_date(jsonResult.FIELD20, "-");
					var vFIELD11 = jsonResult.FIELD11 == "" ? "<BR>" : jsonResult.FIELD11;
					var vFIELD22 = jsonResult.FIELD22 == "" ? "<BR>" : jsonResult.FIELD22;
					var vFIELD23 = jsonResult.FIELD23 == "" ? "<BR>" : jsonResult.FIELD23;

					console.log("2")
					vFIELD23 = format_corp(vFIELD23, "-");

					if (typeof jsonResult.FIELD26 != 'undefined') {
						vFIELD26 = format_time(jsonResult.FIELD26, ":");
					...
					htmlString += "<td class=dwp-center>" + vFIELD20 + "</td>"; //증빙일
					htmlString += "<td class=dwp-center>" + jsonResult.FIELD03 + "</td>"; //계정명
					htmlString += "<td rowspan='2'class=dwp-center>" + jsonResult.FIELD10 + "<BR><BR>" + vFIELD11 + "</td>"; //거래처코드	//거래처명
					htmlString += "<td rowspan='2' class=dwp-center>" + jsonResult.FIELD04 + "<BR><BR>" + vFIELD23 + "</td>"; //코스트센터	//가맹점번호
					htmlString += "<td rowspan='2' class=dwp-center>" + jsonResult.FIELD05 + "<BR><BR>" + vFIELD22 + "</td>"; //코스트센터명	//가맹점명
					htmlString += "<td rowspan='2' class='dwp-right'>" + money1 + "</td>"; //차변(원화)
					htmlString += "<td rowspan='2' class='dwp-right'>" + money2 + "</td>"; //대변(원화)
					htmlString += "</tr>";
					htmlString += "<tr>";
					htmlString += "<td class=dwp-center>" + vFIELD26 + "</td>"; //승인시간
					htmlString += "<td class=dwp-center>" + jsonResult.FIELD08 + "</td><td></td>"; //적요
					//		htmlString += "<td class='clContentTdCenter' colspan='2'>" + jsonResult.FIELD08 + "</td>"; //적요
				...
				htmlString += "<tr>";
				htmlString += "<th class=dwp-center colspan='6'>" + $fn.getCodeMsg("aprv_sub_215.title.a17") + "</th>";
				htmlString += "<td class='dwp-right dwp-bold'>" + commaNum(sum1) + "</td>";		//차변(원화)
				htmlString += "<td class='dwp-right dwp-bold'>" + commaNum(sum2) + "</td>";		//대변(원화)
				htmlString += "</tr>";

				if (_opt.isnew) {
					$("input[name=ed_Data01]", el).val(digitToHangul(sum1));
					$("input[name=ed_Data02]", el).val(commaNum(sum1));
					//$("#sumMoney").text($("input[name=ed_Data01]").val() + "원(\\"+$("input[name=ed_Data02]").val()+")");
				}

				$("#sumMoney", el).html(digitToHangul(sum1) + "원<BR>(₩" + commaNum(sum1) + ")");
			...
			$('[name="role_btn_del"]', el).on('click', function () {
				...
				if ($checkedCheckboxes.length > 0) {
					...
					$fn.alert({//aprv_sub_118.title.a36
						msg: $fn.getCodeMsg("aprv_sub_215.msg.a1")
					});

				}

				_$$.aprv_sub215.updateLinkIndices($doc);
			});
		...
		selectRequestForm1: function (_$doc, _title, _formkey, _addRefer, _callback) {
				...
				buttons: [
					{
						...
						click: function (_$dialog) {
							...
							if ($(selDoc).size() == 0) {
								$fn.alert({//aprv_sub_118.title.a36
									msg: $fn.getCodeMsg("aprv_sub_215.msg.a2")
								});
			...
			function addDraftDocLink(linkText, linkUrl,$doc) {
				...
				if (isDuplicate) {
					$fn.alert({//aprv_sub_118.title.a36
						msg: $fn.getCodeMsg("aprv_sub_215.msg.a3")
					});
