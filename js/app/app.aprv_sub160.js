 ...
 (function (_$$, $) {
    _$$.aprv_sub160 = {
        subdoc: {
            SUBNAME: "sub160"
            , init: function ($doc) {
                var _me = _$$.aprv_sub160.subdoc, opt = $doc.options;
                var el = $doc.element;
                var _$table01 = $("table[name=sub160_Table01]", $doc.element);
                    ...
                    var callback = function (_data) {
                        //if ($('#bodyFld', _el)[0] != undefined) {
                           $dwp.ui.weditor.setHtmlValue("",el);
                            $dwp.ui.weditor.setHtmlValue(_data, el);
                       // }
                    };
                ...
                //원화 체인지시 
                    $("[name=ed_won]", $doc.element).on("change", function () { //1.원화 체인지시
                          

                        _me.comsum($doc);
                        _me.persum($doc);
                        $("[name=ed_won3]", $doc.element).val($("[name=ed_won]", $doc.element).val());
                       
                      
                    
                    });
                    $("[name=ed_won1]", $doc.element).on("change", function () { //2.원화 체인지시
                        _me.comsum1($doc);
                        _me.persum1($doc);
                        $("[name=ed_won4]", $doc.element).val($("[name=ed_won1]", $doc.element).val())
                    
                    });
                    $("[name=ed_won2]", $doc.element).on("change", function () { //3.원화 체인지시
                        _me.comsum2($doc);
                        _me.persum2($doc);
                        $("[name=ed_won5]", $doc.element).val($("[name=ed_won2]", $doc.element).val())
                    
                    });
                    //자기값 체인지해도
                    $("[name=ed_won3]", $doc.element).on("change", function () { //1.화종 체인지시
                        $("[name=ed_won3]", $doc.element).val($("[name=ed_won]", $doc.element).val())
                    
                    });
                    $("[name=ed_won4]", $doc.element).on("change", function () { //2.화종 체인지시
                        $("[name=ed_won4]", $doc.element).val($("[name=ed_won1]", $doc.element).val())
                    
                    });
                    $("[name=ed_won5]", $doc.element).on("change", function () { //3.화종 체인지시
                        $("[name=ed_won5]", $doc.element).val($("[name=ed_won2]", $doc.element).val())
                    
                    });
                    
                    //법인카드 합계
                    $("[name=comcard]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                    
                    });
                    $("[name=comcard_3]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                    
                    });
                    $("[name=comcard_4]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                    
                    });
                    $("[name=comcard_5]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                    
                    });
                    $("[name=comcard_6]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                    
                    });
                    //1환율 온블러시
                    $("[name=percard_sum_so_4]", $doc.element).on("blur", function () { 
                        _me.comsum($doc);
                        _me.persum($doc);
                    
                    });
                     //2환율 온블러시
                     $("[name=percard_sum_so_4_1]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                        _me.persum1($doc);
                    
                    });
                    //3환율 온블러시
                    $("[name=percard_sum_so_4_2]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                        _me.persum2($doc);
                    ...
                    //개인카드 합계
                    $("[name=percard]", $doc.element).on("blur", function () { 
                        _me.persum($doc);
                    
                    });
                    $("[name=percard_3]", $doc.element).on("blur", function () { 
                        _me.persum($doc);
                    
                    });
                    $("[name=percard_4]", $doc.element).on("blur", function () { 
                        _me.persum($doc);
                    
                    });
                    $("[name=percard_5]", $doc.element).on("blur", function () { 
                        _me.persum($doc);
                    
                    });
                    $("[name=percard_6]", $doc.element).on("blur", function () { 
                        _me.persum($doc);
                    
                    });

                      //법인카드 2번졔 합계
                      $("[name=comcard_1]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                    
                    });
                    $("[name=comcard_1_1]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                    
                    });
                    $("[name=comcard_1_2]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                    
                    });
                    $("[name=comcard_1_3]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                    
                    });
                    $("[name=comcard_1_4]", $doc.element).on("blur", function () { 
                        _me.comsum1($doc);
                    
                    });

                     //개인카드 2번째 합계
                    $("[name=percard_1]", $doc.element).on("blur", function () { 
                        _me.persum1($doc);
                    
                    });
                    $("[name=percard_1_1]", $doc.element).on("blur", function () { 
                        _me.persum1($doc);
                    
                    });
                    $("[name=percard_1_2]", $doc.element).on("blur", function () { 
                        _me.persum1($doc);
                    
                    });
                    $("[name=percard_1_3]", $doc.element).on("blur", function () { 
                        _me.persum1($doc);
                    
                    });
                    $("[name=percard_1_4]", $doc.element).on("blur", function () { 
                        _me.persum1($doc);
                    
                    });

                      //법인카드 3번졔 합계
                      $("[name=comcard_2]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                    
                    });
                    $("[name=comcard_2_1]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                    
                    });
                    $("[name=comcard_2_2]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                    
                    });
                    $("[name=comcard_2_3]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                    
                    });
                    $("[name=comcard_2_4]", $doc.element).on("blur", function () { 
                        _me.comsum2($doc);
                    
                    });

                     //개인카드 3번째 합계
                    $("[name=percard_2]", $doc.element).on("blur", function () { 
                        _me.persum2($doc);
                    
                    });
                    $("[name=percard_2_1]", $doc.element).on("blur", function () { 
                        _me.persum2($doc);
                    
                    });
                    $("[name=percard_2_2]", $doc.element).on("blur", function () { 
                        _me.persum2($doc);
                    
                    });
                    $("[name=percard_2_3]", $doc.element).on("blur", function () { 
                        _me.persum2($doc);
                    
                    });
                    $("[name=percard_2_4]", $doc.element).on("blur", function () { 
                        _me.persum2($doc);
            ...
            , save: function ($doc, opt) {
                ...
                var _aopt = $.extend({ actiontype: "" }, opt);
                    ...
                    if ($("[name='Con']",$doc.element).val() == "") {
                            ...
                            //  발주번호를 입력하세요.
                            $fn.alert({msg : $fn.getCodeMsg("aprv_sub_160.msg.a1")});    //사유
