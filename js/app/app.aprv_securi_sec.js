	...
	_$$.aprv_securi_sec = {
		custom: {
			init: function ($did) {
				...
				// 보안서약서 저장처리
				$(".dwp-btn.confirm3", _el).off("click").on("click", function () {
					//debugger;

					// 문구공백이면 진행안햄
					if ($("[name=vsname1]", _el).val() == "") {
					...
					var _syear = $("[name=DYear_3] option:selected", _el).val();
					var _smonth = $("[name=DMonth_3] option:selected", _el).val();
					var _sday = $("[name=DDay_3] option:selected", _el).val();
					...
					$fn.xAjax({
						...
						console.log("처리", data);
						//_Universalid = data.Universalid
						$fn.toast({ msg: _cyear + "년 보안서약서 작성 완료하였습니다." });
