 ...
 (function (_$$, $) {
    _$$.aprv_sub032 = {
        subdoc: {
            SUBNAME: "sub032"
            , init: function ($doc) {
                var _me = _$$.aprv_sub032.subdoc, opt = $doc.options;
                var el = $doc.element;
				//_location Titlename_1
				 if (opt.isnew) {
					...
					$("[name=H_10]", $doc.element).val($("input[name='Titlename_1']",el).val())
				...
				$fn.orgsel($("[name='org']", el), {
				    ...
				    autoseletcomplete: function (event, ui, doc) {
				        ...
				        $("input[name='H_10']").val($dwp.core.lang.getCurMsg(ui.item.value.pos));
				    ...
				    orgselectcomplete: function (dialog, rtndata, doc) {
				        ...
				        $("input[name='H_10']").val($dwp.core.lang.getCurMsg(rtndata.list[0].pos));
						var _url = $dwp.core.util.getProxyUrl('/dwp/com/work/elpisuser.nsf/wcmdpost?createdocument')
						var _param = {
            ...
            , save: function ($doc, opt) {
                 var _me = _$$.aprv_sub032.subdoc, opt = $doc.options;
                var el = $doc.element;
				var startdate=$("[name=H_6_1]", $doc.element).val();
				var enddate=$("[name=H_7_1]", $doc.element).val();

				if (startdate != ""){
					startdate=startdate.substr(0, 10);
					startdate=startdate.split(".")
					$("[name=H_6]", $doc.element).val(startdate[0]+startdate[1]+startdate[2])
				}
				if (enddate != ""){
					enddate=enddate.substr(0, 10);
					enddate=enddate.split(".")
					$("[name=H_7]", $doc.element).val(enddate[0]+enddate[1]+enddate[2])
				}
