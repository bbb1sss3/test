	...
	_$$.aprv_sub221 = {
		...
		load: function ($doc) {
			...
			if (_opt.isedit == false) {
				...
				console.log("읽기모드")
				//선택글자 삭제
				$('span[data-xlang-name^="ed_Payout_"]',el).each(function () {
					var $span = $(this);
				...
				//선택글자 삭제
				$('span[data-xlang-name^="ed_Currency_"]',el).each(function () {
					var $span = $(this);
					var dataValue = $span.attr('data-xlang-value'); // data-xlang-value 속성 가져오기
					var dataLangTxt = $span.attr('data-xlang-txt'); // data-xlang-txt 속성 가져오기
			...
			$('[name="role_btn_add"]', el).on('click', function (e) {
				...
				console.log("품의서추가클릭")
				_$$.aprv_sub221.selectRequestForm1($doc, "품의서", "AF155");
			});
			...
			$('[name="role_btn_del"]', el).on('click', function () {
				...
				if ($checkedCheckboxes.length > 0) {
					...
					$fn.alert({//aprv_sub_118.title.a36
						msg: $fn.getCodeMsg("aprv_sub_221.msg.a1")
					});

				}

				_$$.aprv_sub221.updateLinkIndices($doc);
			});
		...
		selectRequestForm1: function (_$doc, _title, _formkey, _addRefer, _callback) {
				...
				buttons: [
					{
						...
						click: function (_$dialog) {
							...
							if ($(selDoc).size() == 0) {
								$fn.alert({//aprv_sub_118.title.a36
									msg: $fn.getCodeMsg("aprv_sub_221.msg.a2")
								});
			...
			function addDraftDocLink(linkText, linkUrl,$doc) {
				...
				if (isDuplicate) {
					$fn.alert({//aprv_sub_118.title.a36
						msg: $fn.getCodeMsg("aprv_sub_221.msg.a3")
					});
					...
					if (_check_val !== "") {
						for (var j = 0; j < _check_item.length; j++) {
							if (_check_item[j].type === "text") {
								...
								if (_val === "") {
									$fn.alert({
										msg: $fn.getCodeMsg("aprv_sub_221.msg.a4")
									}).done(function () { // <--- 팝업의 '확인' 버튼을 누른 후에 실행되는 부분!
										// 팝업이 닫힌 후에 원하는 input에 포커스를 다시 줘!
										$("input[name='" + _check_item[j].name + i + "']",el).focus();
									});
									return false;
								}
							}
								...
								if (_idx === 0) {
									$fn.alert({//aprv_sub_118.title.a36
										msg: $fn.getCodeMsg("aprv_sub_221.msg.a5")
									}).done(function () { // <--- 팝업의 '확인' 버튼을 누른 후에 실행되는 부분!
										// 팝업이 닫힌 후에 원하는 input에 포커스를 다시 줘!
										$("input[name='" + _check_item[j].name + i + "']",el).focus();
									});
